...
def _route(method, path, body, headers):
    ...
    if p == ["calls"]:
        ...
        only_camp = body.get("only_campaigns") if body else None
        if st:
            where += " AND result=?"
            params.append(st)
        if cid:
    ...
    if p == ["calls", "clear-non-campaign"] and method == "POST":
        if sess["role"] != "admin":
            return {"ok": False, "error": "admin_required"}, 403
        db.q("DELETE FROM calls WHERE campaign_id=0 OR campaign_id IS NULL")
        return OK, 200
    ...
    if p == ["multicom", "check"] and method == "POST":
        if sess["role"] != "admin":
            return {"ok": False, "error": "admin_required"}, 403
        return _multicom_check_route(body)
    if p == ["multicom", "pool-sync"] and method == "POST":
        if sess["role"] != "admin":
            return {"ok": False, "error": "admin_required"}, 403
    ...
    if p == ["amocrm", "check"] and method == "POST":
        if sess["role"] != "admin":
            return {"ok": False, "error": "admin_required"}, 403
        return _amocrm_check_route(body)
    if p == ["amocrm", "users-sync"] and method == "POST":
        if sess["role"] != "admin":
            return {"ok": False, "error": "admin_required"}, 403
...
def _multicom_check_route(body=None):
    from .providers.base import ProviderApiError
    from .telephony import ProviderNotConfigured
    b = body or {}
    mcfg = b.get("multicom") if isinstance(b.get("multicom"), dict) else b
    url = mcfg.get("api_url") if isinstance(mcfg, dict) else None
    key = mcfg.get("api_key") if isinstance(mcfg, dict) else None
    if _is_masked_value(url):
        url = None
    if _is_masked_value(key):
        key = None
    try:
        rep = ENGINE.multicom_check(override_url=url, override_key=key)
